---
title: "Working with React Form and Handling Events"
description: "Form handling in React works a bit differently compared to a regular HTML form. In this step-by-step guide, you will learn this in a simple way."
category: "React"
...
Now, in this part, we will take a look at integrating forms in our React app. Also, you will get to know how to raise and handle events in React.

In [React and Forms](/simple-guide-to-react-form/ "react form" ) we cover how to handle form input fields using modern React Hooks. There, you will learn how the common input types such as checkbox, text, textarea, select input, radio and range work in React.

Until you’ve learned what these Hooks are later in this series you can bookmark the link for later reading. But if you have a basic knowledge of the Hooks, you can read it now to have prior knowledge of how the form works in React.

Let’s move on. Here is the current status of our app.

![React devtools](./images/react-devtools.png)

Now, let’s start by adding the checkboxes.

## Adding checkboxes to the Todo items

You should be familiar with handling this type of input field in a regular HTML form. In React, form handling works a little bit differently.
...
By default, the `input` type (i.e checkboxes) are being handled by the DOM – i.e they have the default HTML behavior; that is why you can toggle the boxes.
...
This means the input checkbox will no longer listen to its internal state (i.e the browser DOM) but rather the state in your app. This is necessary because the component state will not change unless you change it.
